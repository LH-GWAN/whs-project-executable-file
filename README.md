...
- **후방 파일 검사(막음)**: 전방/후방 같이 보기로 후방 파일을 고르면 `core/video_pairs.compare_pair`가
  전방과 같은 녹화인지 본다. 영상 길이 차이가 2초와 5% 중 큰 값을 넘거나, MP4 컨테이너(mvhd)의
  녹화 시각이 10초 넘게 다르면 경고 창을 띄우고 **후방으로 받지 않는다**([다시 고르기] / [전방만
  분석], 사용자 결정). 파일명 짝(_F↔_R)·같은 폴더 여부는 참고 문구만. 길이나 시각을 못 읽는
  파일(AVI는 시각 없음)은 그 항목을 건너뛴다. 실측: VUGERA `_F/_R` 짝 통과, 랜드로버 파일 둘은
  녹화 시각 차이로 차단, INAVI 20초 vs 랜드로버 60초는 길이 차이로 차단.
