...
직접 발급받은 키를 넣을 때는 안내 창의 [키 파일 만들기]로 빈 틀을 만들어 채운다
(메모장으로 직접 만들어도 된다).

### 넣었는데 "키 없음"이 뜰 때

프로그램은 **온라인 모드에서만** 아래 폴더의 키 파일을 찾고, 오프라인 모드에서는 배경지도
...
- **이름은 자유, 확장자는 `.json` 또는 `.txt`.** `online_keys (1).json`, `online_keys.json.txt`
  처럼 저장돼도 인식한다. 폴더 안의 후보 파일을 모두 읽어 보고 안내 창에 "어떤 파일을 왜
  못 썼는지"를 파일별로 적어 준다.
- **값은 영문 소문자·숫자 32자**여야 키로 친다. 예시 문구가 남아 있거나 따옴표·공백이
  섞이면 "키 형식이 아닙니다"로 표시된다. 메모장이 BOM을 붙이거나 워드가 따옴표를 바꿔
  JSON이 깨져도 `kakao_js_key` 뒤의 32자 값은 찾아낸다.
- **프로그램을 다시 켤 필요가 없다.** 파일을 넣고 안내 창의 [다시 확인]을 누르면(또는 2초
  안에) 바로 인식해 온라인 지도로 바꿔 띄운다. 안내 창은 [설정 > 온라인 지도 키 설정 안내]
  에서 언제든 다시 연다.
