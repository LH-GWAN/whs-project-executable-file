...
오프라인 지도 대신 카카오맵을 쓰는 온라인 모드용. `online_keys.example.json`을
`online_keys.json`으로 복사하고 카카오 디벨로퍼스 > 내 애플리케이션 > 앱 키에서
**JavaScript 키**(지도)와 **REST API 키**(좌표→주소)를 넣는다.
...
- **git에 올라가지 않는다**(`.gitignore`). 빌드하는 PC의 이 폴더에만 두면 exe에 번들된다.
- 카카오 디벨로퍼스에서 **카카오맵 > 사용 설정 ON**, 그리고 **[앱] > [플랫폼 키] >
  JavaScript 키 > [JavaScript SDK 도메인]**에 `http://127.0.0.1:48213`,
  `http://127.0.0.1:48214`, `http://127.0.0.1:48215` 등록이 필요하다(프로그램이 이
  포트에서 지도 페이지를 연다). 제품 링크 관리의 "웹 도메인"은 다른 설정이다.
  JavaScript 키가 여러 개면 도메인을 등록한 키를 여기 넣는다.
- 온라인 모드에서는 지도 화면 범위와 주소를 표시할 좌표가 카카오 서버로 전송된다.
  기밀이 중요한 사건은 오프라인을 쓸 것. 무료 한도는 일 지도 30만 건·주소 10만 건이며
  넘으면 프로그램이 안내하고 그날은 오프라인으로 전환해 쓴다.
